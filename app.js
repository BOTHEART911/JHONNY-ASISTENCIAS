...
function viewRegistro(doc) {
  app.innerHTML = `
    ...
    <div class="pad stack">
      ...
      <label class="field"><span>Residencia</span>
        <input class="input" id="rg-fil" type="text" autocomplete="off" placeholder="Filtra la zona…" style="margin-bottom:8px;" />
        <select class="select" id="rg-res"><option value="">Cargando zonas…</option></select>
      </label>
  ...
  pintarZonas();
  $('#rg-fil').addEventListener('input', pintarZonas);
...
function pintarZonas() {
  const sel = $('#rg-res'); if (!sel) return;
  if (!ZONAS) { sel.innerHTML = `<option value="">Cargando zonas…</option>`; return; }
  const f = (($('#rg-fil') || {}).value || '').trim().toUpperCase();
  const previo = sel.value;
  const lista = ZONAS.filter(z => !f || String(z).toUpperCase().includes(f));
  sel.innerHTML = `<option value="">${lista.length ? 'Elige la zona…' : 'Ninguna zona coincide'}</option>` +
    lista.map(z => `<option value="${esc(z)}">${esc(z)}</option>`).join('');
  if (previo && lista.some(z => String(z) === previo)) sel.value = previo;
  else if (lista.length === 1) sel.value = String(lista[0]);
}
